...
```js
let message;
message = 'Hallo'; // gemmer tekststrengen Hallo i variablen message
```
...
Du kan samle det hele - både dekleration og tildeling i én enkelt linje:
...
Som et sidespor, kan det være interessant for dig at vide, at der eksisterer såkaldte [funktionelle](https://en.wikipedia.org/wiki/Functional_programming) programmeringssprog (functional programming) der forbyder, at du ændrer indholdet af variable. Eksempler på sådanne sprog er [Scala](http://www.scala-lang.org/) og [Erlang](http://www.erlang.org/).
